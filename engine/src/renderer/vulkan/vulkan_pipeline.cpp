...
b8 VulkanGraphicsPipelineCreate(
    ...
    DZeroMemory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    VkResult result = vkCreateGraphicsPipelines(context->device.logical_device, VK_NULL_HANDLE, 1,
                                                &pipeline_create_info, context->allocator, &out_pipeline->handle);
    
    if (VulkanResultIsSuccess(result)) {
        DDEBUG("Graphic pipeline create!");
        return true;
    }

    DERROR("vkCreateGraphicPipelines failed with %s.", VulkanResultString(result, true));
    return false;
}
